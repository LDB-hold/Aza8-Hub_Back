...
const seedEnvSchema = zod_1.z.object({
...
const tenantToolActionKeys = seedTools
    .flatMap((tool) => tool.actions.map((action) => action.key))
    .filter((key) => !key.startsWith('catalog.') && !key.startsWith('tenants.') && key !== 'metrics.read');
const viewerToolActionKeys = tenantToolActionKeys.filter((key) => key.endsWith('.read'));
const operatorToolActionKeys = [
    ...new Set([
...
async function seedGlobal(prisma) {
    ...
    for (const pkg of seedPackages) {
        ...
        const missingKeys = pkg.toolActionKeys.filter((key) => !actions.some((found) => found.key === key));
...
async function seedTenant(prisma, input) {
    ...
    if (tenant.status !== 'ativo')
        throw new Error(`Tenant is not active: ${input.tenantId}`);
    let allowedToolActionKeys = tenantToolActionKeys;
    if (input.packageKey) {
        ...
        if (!pkg)
            throw new Error(`Package not found: ${input.packageKey}`);
    ...
    const keyToId = new Map(toolActions.map((row) => [row.key, row.id]));
    async function setGroupPermissions(groupId, toolActionKeys) {
    ...
    await setGroupPermissions(groupOperator.id, operatorToolActionKeys.filter((key) => allowedToolActionKeys.includes(key)));
    await setGroupPermissions(groupViewer.id, viewerToolActionKeys.filter((key) => allowedToolActionKeys.includes(key)));
    if (input.adminUser) {
...
async function main() {
    ...
    try {
        ...
        if (args.tenantId) {
            const adminUser = args.adminEmail && args.adminName
